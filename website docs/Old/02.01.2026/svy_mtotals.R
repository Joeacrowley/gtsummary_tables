svy_mtotals <- 
function (outcomes, data, caption = "Weighted percentages") 
{
    results <- map(outcomes, function(outcome) {
        table_data <- data %>% mutate(across(all_of(outcome), 
            ~fct_drop(.x), .names = "the_merging_variable")) %>% 
            mutate(across(all_of(outcome), ~case_when(!is.na(.x) ~ 
                "Unweighted base"), .names = "base"))
        var_label(table_data[["variables"]]["base"]) <- ""
        result <- tbl_svysummary(data = table_data, include = c(the_merging_variable, 
            base), statistic = list(all_categorical() ~ "{p}", 
            base ~ "{n_unweighted}"), label = list(the_merging_variable ~ 
            " "), missing = "no", digits = everything() ~ 0) %>% 
            modify_header(stat_0 = paste0("**", var_label(data[["variables"]][[outcome]]), 
