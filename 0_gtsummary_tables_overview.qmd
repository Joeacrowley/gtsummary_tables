...
### Sourcing functions from GitHub

```{r, error =T}
source_gtsummary_helpers <- function(
    repo = "Joeacrowley/gtsummary_tables",
    ref  = "main") {
  api <- sprintf("https://api.github.com/repos/%s/contents?ref=%s", repo, ref)
  contents <- jsonlite::fromJSON(api)
  r_urls <- contents$download_url[
    contents$type == "file" & grepl("\\.R$", contents$name)
  ]
  invisible(lapply(r_urls, function(u) {
    message("source(): ", basename(u))
    source(u)
  }))
}

source_gtsummary_helpers()

```

<br>

### Sourcing functions at work

```{r}
load_gtsummary_fns <- function(
    ddir = "F:/Github/gtsummary_tables/",
    exclude_pattern = "_interim",
    verbose = TRUE
) {
  file_paths <- list.files(path = ddir, pattern = "\\.R$", full.names = FALSE, all.files = TRUE) %>%
    grep(exclude_pattern, ., value = TRUE, invert = TRUE)
  
  if (verbose) message("Loading ", length(file_paths), " file(s):\n", paste0("  ", file_paths, collapse = "\n"))
