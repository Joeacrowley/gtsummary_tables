...
function (df, align = 1) 
{
    ...
    if (length(raw_caption) == 0 || is.na(raw_caption)) {
        ...
        caption_divs <- str_match_all(raw_caption, "<div[^>]*>(.*?)</div>")[[1]][, 
            2]
        caption_parts <- caption_divs %>% str_replace_all("<[^>]*>", 
            " ") %>% str_squish()
    ...
    ht <- set_top_border(ht, row = c(1, first_body_row), col = everywhere, 
        value = 2)
    ht <- set_bottom_border(ht, row = c(final_body_row), col = everywhere, 
        value = 2)
    if (any(whether_any_sample_sizes_below_table)) {
    ...
    if (length(footer_rows) > 0) {
        ...
        for (r in footer_rows) {
            ...
            if (str_detect(raw_text, "<br|<b>")) {
                footer_lines <- raw_text %>% str_split_1("(?i)<br\\s*/?>") %>% 
                  trimws()
                ...
                new_rows <- as_hux(matrix(footer_lines_clean, 
                  nrow = n_lines, ncol = ncol(ht)))
                for (rr in seq_len(n_lines)) new_rows <- merge_across(new_rows, 
                  rr, everywhere)
                for (rr in which(is_bold_line)) new_rows <- set_bold(new_rows, 
                  rr, everywhere, TRUE)
                new_footer_blocks[[length(new_footer_blocks) + 
                  1]] <- new_rows
            }
            else {
    ...
    if (!(length(caption_parts) == 1 && caption_parts[1] == " ")) {
        ...
        empty_hux <- set_header_rows(empty_hux, seq_len(n_parts), 
            TRUE)
        ...
        ht <- set_bottom_border(ht, row = n_parts, col = everywhere, 
            value = 2)
    }
