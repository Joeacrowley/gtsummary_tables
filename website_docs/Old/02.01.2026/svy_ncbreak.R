    ...
    suppressWarnings({
        suppressMessages({
            ...
            for (i in 1:length(outcomes)) {
                ...
                nvar <- nest
                df_tmp <- data %>% select(all_of(c(outcomes, 
                  by_var, nvar))) %>% filter(!is.na(.data[[nvar]])) %>% 
                ...
                perc <- df_tmp %>% tbl_strata(strata = {
                ...
                }, ~.x %>% tbl_svysummary(include = all_of(c(o_var, 
                  base_name)), by = .data[[by_var]], missing = "no", 
                  statistic = list(all_categorical() ~ "{p}", 
                    base_name ~ "{n_unweighted}"), digits = list(all_categorical() ~ 
                  ...
                  modify_footnote(all_stat_cols() ~ NA), .header = "**{strata}**, N = {n_unweighted}")
                perc <- perc %>% modify_table_body(~.x %>% dplyr::mutate(label = case_when(label == 
                  o_var ~ var_label(data[["variables"]][[o_var]]), 
                  label == base_name ~ " ", TRUE ~ label)))
                cross_break[[i]] <- perc
            }
            table <- cross_break %>% tbl_stack() %>% modify_caption(paste0("<div style='text-align: left; font-weight: bold; color: black'>", 
                "Crosstabulation by ", var_label(data[["variables"]][[by_var]]), 
                ", \n                 nested by ", var_label(data[["variables"]][[nvar]]), 
