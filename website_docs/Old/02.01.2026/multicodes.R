multicodes <- 
function (df, vars, ci = FALSE, caption = "Unweighted percentages", 
    breaks = NULL, sig = F) 
{
    ...
    if (chk1 == TRUE & chk2 == TRUE) {
        ...
        variable_label_stem <- df %>% pull(vars[1]) %>% var_label() %>% 
            str_split_i(., ": ", 1)
        df <- revise_labels(df, vars = vars)
        tbl <- df %>% tbl_summary(include = all_of(var_set), 
            type = list(unweighted_base ~ "categorical"), missing = "no", 
            statistic = list(all_dichotomous() ~ "{p}", all_categorical() ~ 
                "{p}", unweighted_base ~ "{n}"), digits = list(all_dichotomous() ~ 
                1))
        if (ci == TRUE) {
        ...
        if (!is.null(breaks)) {
            suppressWarnings({
                suppressMessages({
                  brk_tbls <- map(breaks, ~df %>% tbl_summary(include = all_of(var_set), 
